...
def printGrid(walls, init_blizzards, width, height, points = {}, step = 0):
    ...
    for y in range(-1 , height+1):
        ...
        for x in range(-1 ,width+1):
            blizz_icons = []
            if (x,y,0,1) in blizzards: blizz_icons.append('v')
            if (x,y,0,-1) in blizzards: blizz_icons.append('^')
            if (x,y,-1,0) in blizzards: blizz_icons.append('<')
            if (x,y,1,0) in blizzards: blizz_icons.append('>')
            ...
            elif (x,y) in points: line.append("X")
            else: line.append('.')
...
def part_one():
    """Solution for Part 1"""
    walls, blizzards, empties = parse_input()
    start = empties[0]
    end = empties[-1]
    width = end[0] + 1
    height = end[1]
    ...
    result = bfs(blizzards, walls, start, end, width, height)
    return result


def bfs(blizzards, walls, start, end, width, height, step=0):
